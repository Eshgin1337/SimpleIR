# Parser.py - Fixed s() call in visitWhile

import sys
from antlr4 import *
# Need access to ParserRuleContext for isinstance check
from antlr4.tree.Tree import TerminalNodeImpl, ErrorNodeImpl
from antlr4 import ParserRuleContext # Import base class
...
class Printer(WhileVisitor):
    ...
    def visitCompound(self, ctx: WhileParser.CompoundContext):
        ...
        # Iterate through generic children of the INNER context
    ...
    def visitIf(self, ctx: WhileParser.IfContext):
        ...
        current_indent = self.get_indent()
        self.increase_indent()
        # Rule if_stmt has s TWICE -> use index 0 and 1
    ...
    def visitWhile(self, ctx: WhileParser.WhileContext):
        ...
        current_indent = self.get_indent()
        self.increase_indent()
        # Access the body statement (single 's') using NO index
        # Rule while_stmt has s ONCE -> use s()
        body_str = self.visit(inner_ctx.s()) # <<< CORRECTED: Removed index 0
        self.decrease_indent()
        # Format according to grammar (no trailing 'done')
    ...
    def visitENot(self, ctx: WhileParser.ENotContext):
        ...
        if isinstance(operand_ctx, (WhileParser.EBinOpAndOrContext,
                                    WhileParser.ECompContext,
                                    WhileParser.EAddContext,
                                    WhileParser.EMultContext)):
