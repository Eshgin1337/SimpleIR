# Interpreter.py - Fixed getText() call for size token
...
# 'var_name': {
#     'type': 'int' | 'bool' | 'array',
#     'value': int | bool | dict{int_idx: int/bool}, # Current value / Array elements (sparse dict)
#     'size': None | int,                           # For arrays
#     'element_type': None | 'int' | 'bool'         # For arrays
# }
symtab = {}
...
def get_value_type(value):
    """Determines the language type ('int' or 'bool') of a Python value."""
    if isinstance(value, bool): return 'bool'
    if isinstance(value, int): return 'int'
    return 'unknown' # Should not happen with current type checks
...
class Interpreter(WhileVisitor):
    ...
    # --- Declaration Visitor ---
    # Corresponds to 's: declaration # DeclarationStmt' in grammar
    def visitDeclarationStmt(self, ctx: WhileParser.DeclarationStmtContext):
        ...
        type_name = decl_ctx.typeName.text # 'int' or 'bool'

        if var_name in symtab:
            runtime_error(f"Variable '{var_name}' already declared.")

        size_ctx = decl_ctx.size # This is the NUM token if present

        if size_ctx: # It's an array declaration
            try:
                # --- CORRECTED LINE ---
                size = int(size_ctx.text) # Use .text for token objects
                # ----------------------
                if size <= 0:
                     runtime_error(f"Array size must be positive for '{var_name}'.")
            except ValueError:
                 runtime_error(f"Invalid array size '{size_ctx.text}' for '{var_name}'.")

            # Store array info, value is initialized empty (sparse)
            symtab[var_name] = {
                'type': 'array',
                'value': {},
                'size': size,
             ...
             symtab[var_name] = {
                'type': type_name,
                'value': DEFAULT_VALUES[type_name], # Initialize scalar
                'size': None,
                'element_type': None
            }
        return None # Declarations don't produce a value
    ...
    def visitCompound(self, ctx: WhileParser.CompoundContext):
        ...
        if hasattr(inner_ctx, 's') and inner_ctx.s(0):
            num_statements = 1
            if hasattr(inner_ctx, 'SEMI') and inner_ctx.SEMI():
                 num_statements += len(inner_ctx.SEMI())
    ...
    def visitIf(self, ctx: WhileParser.IfContext):
        ...
        if condition:
            self.visit(inner_ctx.s(0))
        elif inner_ctx.s(1):
            self.visit(inner_ctx.s(1))
        return None
    ...
    def visitWhile(self, ctx: WhileParser.WhileContext):
        ...
        while True:
            ...
            self.visit(inner_ctx.s()) # Use s() here - correct based on grammar
        return None
    ...
    def visitEBinOpComp(self, ctx: WhileParser.EBinOpCompContext):
        ...
        op = ctx.op.text
        # Allow comparing int==int or bool==bool
        if get_value_type(left) != get_value_type(right):
             runtime_error(f"Cannot compare values of different types: {get_value_type(left)} {op} {get_value_type(right)}")
        # Only allow <, <=, >, >= for integers
        if op in ['<', '<=', '>', '>='] and get_value_type(left) != 'int':
             runtime_error(f"Comparison operator '{op}' only supports integers, got {get_value_type(left)}.")
    ...
    def visitArrayAccess(self, ctx: WhileParser.ArrayAccessContext):
        ...
        # Return element value, using default (0/False) if index hasn't been assigned yet
        element_value = array_info['value'].get(index_value, DEFAULT_VALUES[element_type])
        # Optional: Check if stored value matches element type (paranoid check)
        # if get_value_type(element_value) != element_type: runtime_error(...)
